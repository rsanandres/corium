...
// CoriumMonitorAlertReconciler reconciles a CoriumMonitorAlert object
type CoriumMonitorAlertReconciler struct {
	client.Client
	Scheme   *runtime.Scheme
	Recorder record.EventRecorder
}

// +kubebuilder:rbac:groups=monitor.corium.io,resources=coriummonitoralerts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=monitor.corium.io,resources=coriummonitoralerts/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=monitor.corium.io,resources=coriummonitoralerts/finalizers,verbs=update
// +kubebuilder:rbac:groups=monitor.corium.io,resources=coriummonitorcollectors,verbs=get;list;watch
// +kubebuilder:rbac:groups="",resources=configmaps,verbs=get;list;watch
// +kubebuilder:rbac:groups="",resources=events,verbs=create;patch

func (r *CoriumMonitorAlertReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !alert.Spec.Enabled {
		alert.Status.AlertStatus = alertStatusDisabled
	...
	if err := r.Get(ctx, types.NamespacedName{
		...
		alert.Status.AlertStatus = alertStatusError
		meta.SetStatusCondition(&alert.Status.Conditions, metav1.Condition{
	...
	if err := r.Status().Update(ctx, alert); err != nil {
		logger.Error(err, "unable to update CoriumMonitorAlert status")
		return ctrl.Result{}, err
	}
...
func (r *CoriumMonitorAlertReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&monitorv1alpha1.CoriumMonitorAlert{}).
		Named("coriummonitoralert").
		Complete(r)
}
