...
var _ = Describe("CoriumMonitorConfig Controller", func() {
	...
	Context("When reconciling a valid enabled config", func() {
		...
		BeforeEach(func() {
			resource := &monitorv1alpha1.CoriumMonitorConfig{}
			...
			if err != nil && errors.IsNotFound(err) {
				resource = &monitorv1alpha1.CoriumMonitorConfig{
					...
					Spec: monitorv1alpha1.CoriumMonitorConfigSpec{
						...
						StorageConfig: monitorv1alpha1.StorageConfig{
							Type:     "prometheus",
		...
		It("should set status to Active and add finalizer", func() {
			reconciler := &CoriumMonitorConfigReconciler{
				Client:   k8sClient,
			...
			updated := &monitorv1alpha1.CoriumMonitorConfig{}
